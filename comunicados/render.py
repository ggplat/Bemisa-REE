...
def _fmt_mcap(usd_millions: Optional[float]) -> Optional[str]:
    if usd_millions is None:
        return None
    if usd_millions >= 1000:
        return f"USD {usd_millions / 1000:.2f}B"
    return f"USD {usd_millions:.0f}M"
...
def _build_company(company: Company, anns: list[Announcement], *,
                    selected: bool, mcap_label: Optional[str] = None) -> dict:
...
def build_context(companies: list[Company],
                  ...
                  *, updated: Optional[dt.datetime] = None,
                  market_caps: Optional[dict] = None) -> dict:
    ...
    for i, company in enumerate(companies):
        ...
        mcap = market_caps.get(company.ticker)
        mcap_label = _fmt_mcap(mcap.usd_millions) if mcap else None
        company_views.append(_build_company(company, anns, selected=(i == 0), mcap_label=mcap_label))
